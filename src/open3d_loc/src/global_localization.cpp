...
private:
    ...
    std::string initialpose_preset_;
    std::string active_region_;
    Eigen::Vector3d map_crop_extent_ = Eigen::Vector3d(8.0, 8.0, 3.0);
    Eigen::Vector3d scan_crop_extent_ = Eigen::Vector3d(6.0, 6.0, 3.0);
                                             ...
                                             tf_listener_(std::make_shared<tf2_ros::TransformListener>(tf_buffer_))
{
    ...
    this->declare_parameter<std::vector<double>>("initialpose_default", std::vector<double>(6, 0.0));
    this->declare_parameter<std::string>("initialpose_preset", "start_origin");
    this->declare_parameter<std::vector<double>>("crop_box.map_extent", std::vector<double>{8.0, 8.0, 3.0});
    this->declare_parameter<std::vector<double>>("crop_box.scan_extent", std::vector<double>{6.0, 6.0, 3.0});
    this->declare_parameter<std::string>("active_region", "global");
    this->declare_parameter<double>("dis_updatemap", 1);
    this->declare_parameter<std::string>("world", "");
    this->declare_parameter<std::string>("loc_map", "");
    ...
    this->get_parameter("initialpose_preset", initialpose_preset_);
    this->get_parameter("active_region", active_region_);

    std::vector<double> legacy_initialpose;
    std::vector<double> initialpose_default;
    std::vector<double> map_extent_param;
    std::vector<double> scan_extent_param;
    std::string world_name;
    std::string loc_map_name;
    this->get_parameter("initialpose", legacy_initialpose);
    this->get_parameter("initialpose_default", initialpose_default);
    this->get_parameter("crop_box.map_extent", map_extent_param);
    this->get_parameter("crop_box.scan_extent", scan_extent_param);
    this->get_parameter("world", world_name);
    this->get_parameter("loc_map", loc_map_name);

    auto normalize_initialpose = [&](const std::vector<double> &pose, const std::vector<double> &fallback, const std::string &name)
    {
        if (pose.size() == 6)
        {
            return pose;
        ...
        if (!pose.empty())
        {
            RCLCPP_WARN(this->get_logger(), "parameter %s must contain 6 values, fallback will be used", name.c_str());
        }
        return fallback;
    };
    auto vector_to_extent = [&](const std::vector<double> &extent, const Eigen::Vector3d &fallback, const std::string &name)
    {
        if (extent.size() == 3)
        {
            return Eigen::Vector3d(extent[0], extent[1], extent[2]);
        }
        if (!extent.empty())
        {
            RCLCPP_WARN(this->get_logger(), "parameter %s must contain 3 values, fallback will be used", name.c_str());
        }
        return fallback;
    };

    std::vector<double> zero_pose(6, 0.0);
    initialpose_default = normalize_initialpose(initialpose_default, zero_pose, "initialpose_default");
    initialpose_ = normalize_initialpose(legacy_initialpose, initialpose_default, "initialpose");

    const std::string initialpose_preset_param = "initialpose_presets." + initialpose_preset_;
    this->declare_parameter<std::vector<double>>(initialpose_preset_param, initialpose_default);
    std::vector<double> preset_pose = initialpose_default;
    this->get_parameter(initialpose_preset_param, preset_pose);
    initialpose_ = normalize_initialpose(preset_pose, initialpose_, initialpose_preset_param);

    map_crop_extent_ = vector_to_extent(map_extent_param, map_crop_extent_, "crop_box.map_extent");
    scan_crop_extent_ = vector_to_extent(scan_extent_param, scan_crop_extent_, "crop_box.scan_extent");

    const std::string region_prefix = "regions." + active_region_;
    this->declare_parameter<std::vector<double>>(region_prefix + ".map_extent",
                                                 std::vector<double>{map_crop_extent_.x(), map_crop_extent_.y(), map_crop_extent_.z()});
    this->declare_parameter<std::vector<double>>(region_prefix + ".scan_extent",
                                                 std::vector<double>{scan_crop_extent_.x(), scan_crop_extent_.y(), scan_crop_extent_.z()});
    this->declare_parameter<double>(region_prefix + ".loc_frequence", loc_frequence_);
    this->declare_parameter<double>(region_prefix + ".dis_updatemap", dis_updatemap_);
    this->declare_parameter<double>(region_prefix + ".threshold_fitness_init", threshold_fitness_init_);
    this->declare_parameter<double>(region_prefix + ".threshold_fitness", threshold_fitness_);

    std::vector<double> region_map_extent;
    std::vector<double> region_scan_extent;
    this->get_parameter(region_prefix + ".map_extent", region_map_extent);
    this->get_parameter(region_prefix + ".scan_extent", region_scan_extent);
    this->get_parameter(region_prefix + ".loc_frequence", loc_frequence_);
    this->get_parameter(region_prefix + ".dis_updatemap", dis_updatemap_);
    this->get_parameter(region_prefix + ".threshold_fitness_init", threshold_fitness_init_);
    this->get_parameter(region_prefix + ".threshold_fitness", threshold_fitness_);

    map_crop_extent_ = vector_to_extent(region_map_extent, map_crop_extent_, (region_prefix + ".map_extent"));
    scan_crop_extent_ = vector_to_extent(region_scan_extent, scan_crop_extent_, (region_prefix + ".scan_extent"));
    ...
    RCLCPP_INFO(this->get_logger(),
                "world=%s loc_map=%s initialpose_preset=%s active_region=%s",
                world_name.c_str(), loc_map_name.c_str(), initialpose_preset_.c_str(), active_region_.c_str());
    RCLCPP_INFO(this->get_logger(),
                "map_crop_extent=[%.2f %.2f %.2f] scan_crop_extent=[%.2f %.2f %.2f]",
                map_crop_extent_.x(), map_crop_extent_.y(), map_crop_extent_.z(),
                scan_crop_extent_.x(), scan_crop_extent_.y(), scan_crop_extent_.z());
    ...
    {
        RCLCPP_WARN(this->get_logger(), "motion_link TF unavailable during initialization, using identity");
        mat_baselink2motionlink_ = Eigen::Matrix4d::Identity();
    }
...
void GloabalLocalization::LocalizationInitialize()
{
    ...
    /// 固定感知子图/历史地图子图大小
    OBB_map->extent_ = map_crop_extent_;
    OBB_map->color_ = Eigen::Vector3d(1, 0.5, 0);
    OBB_scan->extent_ = scan_crop_extent_;
...
{
    RCLCPP_INFO(this->get_logger(), "wait for Odometry");
