...
tf2::Transform tf_imu_from_base = tf2::Transform::getIdentity();
tf2::Transform tf_pub_odom_from_internal_odom = tf2::Transform::getIdentity();
bool mount_transform_initialized = false;
...
{
  if (mount_transform_initialized) {
    return true;
  }

  try {
    const auto imu_from_base_msg =
      tf_buffer.lookupTransform("imu_link", "base_link", tf2::TimePointZero);
    tf2::fromMsg(imu_from_base_msg.transform, tf_imu_from_base);
    mount_transform_initialized = true;
    return true;
  } catch (tf2::TransformException & ex) {
    RCLCPP_ERROR(logger, "Failed to lookup transform from base_link to imu_link: %s", ex.what());
    return false;
  }
}
  ...
  const rclcpp::Logger & logger)
{
  ...
  const tf2::Transform tf_internal_odom_to_base =
    tf_internal_odom_to_imu * tf_imu_from_base;
  ...
  const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr & pubLaserCloudFullRes)
{
  ...
  PointCloudXYZI::Ptr init_map_cloud(new PointCloudXYZI(*init_feats_world));
  transform_cloud_to_published_odom(init_map_cloud);
  sensor_msgs::msg::PointCloud2 laserCloudmsg;
  pcl::toROSMsg(*init_map_cloud, laserCloudmsg);
  ...
  const tf2_ros::Buffer & tf_buffer, const rclcpp::Logger & logger)
{
  ...
  if (publish_odometry_without_downsample) {
    odometry_msg.header.stamp = get_ros_time(time_current);
  } else {
    odometry_msg.header.stamp = get_ros_time(lidar_end_time);
  }
  odometry_msg.header.frame_id = "odom";
  odometry_msg.child_frame_id = "base_link";
  set_posestamp(odometry_msg.pose.pose);
  set_twist(odometry_msg.twist);
  ...
  tf2::fromMsg(odometry_msg.pose.pose, tf_internal_odom_to_imu);
  if (!ensure_mount_transforms_initialized(tf_buffer, logger)) {
  ...
  latest_published_base_pose = odometry_msg.pose.pose;
  latest_published_base_pose_valid = true;

  tf_br.sendTransform(transform_stamped);
  pubOdomAftMapped.publish(odometry_msg);
}
...
int main(int argc, char ** argv)
{
  ...
  auto sub_imu =
    nh->create_subscription<sensor_msgs::msg::Imu>(imu_topic, rclcpp::SensorDataQoS(),
      [&nh, &tf_buffer, &tf_broadcaster](const sensor_msgs::msg::Imu &msg_in) {
        imu_cbk(*nh, msg_in, *tf_buffer, *tf_broadcaster);
      });
  ...
  while (rclcpp::ok()) {
    ...
    if (sync_packages(Measures)) {
      if (flg_reset) {
        ...
        flg_reset = false;
        init_map = false;
        published_odom_alignment_initialized = false;
        latest_published_base_pose_valid = false;
      ...
      if (feats_undistort->empty()) {
        RCLCPP_WARN(logger, "No point after body-frame crop, skip this scan!\n");
        continue;
      }
