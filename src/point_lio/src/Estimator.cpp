...
double farthest_neighbor_sq_distance(
  const PointType & point_world,
  const PointVector & points_near)
{
  double max_sq = 0.0;
  for (const auto & point_near : points_near) {
    const double dx = static_cast<double>(point_world.x) - point_near.x;
    const double dy = static_cast<double>(point_world.y) - point_near.y;
    const double dz = static_cast<double>(point_world.z) - point_near.z;
  ...
  VF(4) & pabcd)
{
  ...
  if (max_match_sq_dist > 0.0 &&
      farthest_neighbor_sq_distance(point_world, points_near) > max_match_sq_dist)
  {
    return false;
  }
...
void h_model_input(state_input &s, Eigen::Matrix3d cov_p, Eigen::Matrix3d cov_R, esekfom::dyn_share_modified<double> &ekfom_data)
{
	...
	for (int j = 0; j < time_seq[k]; j++)
	{
		...
		p_world << point_world_j.x, point_world_j.y, point_world_j.z;
		{
			...
			if (!plane_candidate_is_valid(point_world_j, points_near, pabcd))
			{
