...
class R1Mode1ChallengeBlueTest(R1Mode1ChallengeBlue):
    ...
    STARTUP_MODE3_NO_REPLY_DELAY_SEC = 118.0
    ...
    def __init__(self, node_name: str = "r1_mode1_challenge_blue_test"):
        ...
        self._startup_mode3_no_reply_timer = None
        self._startup_mode3_no_reply_sent = False
        ...
        self.declare_parameter(
            "startup_mode3_no_reply_delay_sec",
            self.STARTUP_MODE3_NO_REPLY_DELAY_SEC,
        )
        ...
        self.startup_mode3_no_reply_delay_sec = max(
            0.1,
            float(self.get_parameter("startup_mode3_no_reply_delay_sec").value),
        )
        ...
        self._startup_mode3_no_reply_timer = self.create_timer(
            self.startup_mode3_no_reply_delay_sec,
            self.send_startup_mode3_no_reply,
        )
        self.get_logger().info(
            ...
            f"startup_mode3_no_reply_delay={self.startup_mode3_no_reply_delay_sec:.1f}s "
    ...
    def send_startup_mode3_no_reply(self):
        if self._startup_mode3_no_reply_timer is not None:
            self._startup_mode3_no_reply_timer.cancel()
            self._startup_mode3_no_reply_timer = None

        with self.lock:
            if self._startup_mode3_no_reply_sent:
                return
            self._startup_mode3_no_reply_sent = True

        if not (self.serial and self.serial.is_open):
            self.get_logger().warn("启动118s定时命令3发送失败：串口不可用")
            return

        frame = build_vision_frame(0x01, 0x03, 0, 0)
        try:
            self.serial.reset_output_buffer()
            self.serial.write(frame)
            self.serial.flush()
            self.get_logger().info(
                "启动118s定时任务：已发送区域1模式3串口帧（need_return=0，无需回包）"
            )
        except Exception as exc:
            self.get_logger().error(f"启动118s定时命令3发送失败: {exc}")
