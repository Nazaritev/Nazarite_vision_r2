...
class R2_mode1_blue(R2_mode1):
    ...
    def barcode_callback(self, msg):
        ...
        with self.lock:
            ...
            self.set_chassis_mode(
                ChassisMode.HOLD,
                "蓝方二维码2，执行串口序列3->10，收到10回包后直接顶靠下一个抓取点",
            )
            self.send_serial("3")
            self.stop_serial("3")
            threading.Timer(
    ...
    def send_blue_qr2_cmd10_after_delay(self):
        with self.lock:
            ...
            self.send_serial("10")
            self.get_logger().info(
                "蓝方二维码2：命令3后延时结束，已发送命令10，等待命令10回包"
            )
    ...
    def handle_blue_qr2_serial_frame(self, frame: dict) -> bool:
        ...
        with self.lock:
            ...
            if step == "waiting_10":
                ...
                self.get_logger().info(
                    f"蓝方二维码2：收到命令10完成回包，跳过抓取点 {next_index} 导航；"
    ...
    def navigate_to_grab_point(self, index: int):
        ...
        self.publish_mode1_and_enable_contact_logic(
            f"蓝方抓取点 {index} 跳过导航，直接开启顶靠逻辑"
        )
        self.set_chassis_mode(
            ChassisMode.CONTACT_APPROACH,
            f"蓝方抓取点 {index} 不发布导航点，直接固定顶靠",
        )
