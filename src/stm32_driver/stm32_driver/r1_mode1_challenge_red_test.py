...
class R1Mode1ChallengeRedTest(R1Mode1ChallengeRed):
    CHALLENGE_BARCODE_POINTS = (
        Point3d(-0.80, 0.21, 0.0, 0.0),#0.21
    )
    ...
    def __init__(self, node_name: str = "r1_mode1_challenge_red_test"):
        ...
        self.declare_parameter("contact_approach_timeout_sec", 1.5)
    ...
    def start_initial_navigation(self):
        super().start_initial_navigation()
    ...
    def force_mode9_and_publish_mode2(self):
        with self.lock:
            ...
            self.send_serial("11")
            self.get_logger().warn(
                "红方挑战测试：启动两分钟计时到达，已强制发送区域1模式11，等待回包后发布 /mode=2"
            )
        ...
        with self.lock:
            if (
                ...
                self.publish_mode2_and_release_hold(
                    "红方挑战测试：启动两分钟超时，区域1模式11回包完成，发布 /mode=2"
                )
                self.stop_serial_reader_after_mode2()
                self.get_logger().warn(
                    "红方挑战测试：超时区域1模式11已收到完成回包，已发布 /mode=2，状态 -> FINISHED"
                )
                return
